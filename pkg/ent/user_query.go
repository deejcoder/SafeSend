...
func (uq *UserQuery) QueryEntities() *EntityQuery {
	query := &EntityQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withEntities; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*User)
		for i := range nodes {
			if nodes[i].entity_users == nil {
				continue
			}
			fk := *nodes[i].entity_users
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "entity_users" returned %v`, n.ID)
			}
			for i := range nodes {
