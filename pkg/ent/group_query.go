...
func (gq *GroupQuery) QueryEntities() *EntityQuery {
	query := &EntityQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if gq.withUsers != nil || gq.withEntities != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := gq.withEntities; query != nil {
		...
		nodeids := make(map[uuid.UUID][]*Group)
		for i := range nodes {
			if nodes[i].entity_groups == nil {
				continue
			}
			fk := *nodes[i].entity_groups
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "entity_groups" returned %v`, n.ID)
			}
			for i := range nodes {
