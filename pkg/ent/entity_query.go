...
func (eq *EntityQuery) QueryUsers() *UserQuery {
...
func (eq *EntityQuery) QueryGroups() *GroupQuery {
...
func (eq *EntityQuery) WithUsers(opts ...func(*UserQuery)) *EntityQuery {
	query := &UserQuery{config: eq.config}
...
func (eq *EntityQuery) WithGroups(opts ...func(*GroupQuery)) *EntityQuery {
	query := &GroupQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Entity.Query().
//		GroupBy(entity.FieldEntityType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (eq *EntityQuery) sqlAll(ctx context.Context) ([]*Entity, error) {
	...
	if query := eq.withUsers; query != nil {
		...
		query.Where(predicate.User(func(s *sql.Selector) {
			s.Where(sql.InValues(entity.UsersColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.entity_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "entity_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "entity_users" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := eq.withGroups; query != nil {
		...
		query.Where(predicate.Group(func(s *sql.Selector) {
			s.Where(sql.InValues(entity.GroupsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.entity_groups
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "entity_groups" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "entity_groups" returned %v for node %v`, *fk, n.ID)
			}
