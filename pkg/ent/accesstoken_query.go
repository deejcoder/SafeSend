...
func (atq *AccessTokenQuery) IDs(ctx context.Context) ([]uuid.UUID, error) {
	var ids []uuid.UUID
...
//		GroupBy(accesstoken.FieldTokenProvider).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.AccessToken.Query().
//		Select(accesstoken.FieldTokenProvider).
//		Scan(ctx, &v)
//
...
func (atq *AccessTokenQuery) sqlAll(ctx context.Context) ([]*AccessToken, error) {
	...
	if query := atq.withUsers; query != nil {
		...
		ids := make(map[uuid.UUID]*AccessToken, len(nodes))
		...
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*AccessToken)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(accesstoken.UsersPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, atq.driver, _spec); err != nil {
